...
def getcloudwatchdata(parameter,file_name):
    logfile = open('/home/qa/scripts/cloudwatch/cloudwatch.log','a+')
...
#    pdb.set_trace()
    ...
    elif re.search('Memoryfree',parameter):
        TotalMemory=config.get(parameter,'TotalMemory')
        TotalMemory_bytes = float(TotalMemory)*1024*1024*1024
        percent_free = round(float(result))/TotalMemory_bytes*100
        result_format = str(round(percent_free,2))
    ...
    try:
        print result_format
        os.system('echo %s > /home/qa/scripts/cloudwatch/cloudwatch/%s' % (result_format,parameter))
    except Exception,e:
...
if __name__ == '__main__':
    ...
    #     getcloudwatchdata(sys.argv[1])
        ...
        for secetion in all_secetions:
            print secetion
            getcloudwatchdata(secetion,file_name)
